...
export function PayModal({ open, setOpen, payId }) {
  const [sending, setSending] = useState(false);

  const onPayClick = async () => {
    try {
      setSending(true);
      const res = await payOrder(payId);
      ...
      console.log('Error caught - payOrder()', error);
    }
  };
    ...
    <div>
      {open && (
        <div className="absolute z-40 top-0 left-0 h-screen w-screen bg-[rgba(0,0,0,.7)] flex justify-center pt-10">
          <div className="border border-[#16a34a] rounded-lg h-fit w-1/3 p-5 text-center relative">
            <div
            ...
            <p className="mt-5">Order was sent successfully!</p>
            <p>Pay the order to avoid delay.</p>
            <button
              type="button"
              onClick={() => onPayClick()}
              disabled={sending}
